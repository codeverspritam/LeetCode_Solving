...
# Q6: [2260. Minimum Consecutive Cards to Pick Up](https://leetcode.com/problems/minimum-consecutive-cards-to-pick-up)

**Difficulty:** Medium  
**Companies:** Amazon, Google, Uber, Goldman Sachs

Ye problem **Sliding Window** aur **Hashing** ke core concepts ko test karne ke liye interviewers ka favorite hai:

- **Amazon**: Unke interviews mein "shortest subarray with X property" wale questions bahut common hote hain.
- **Google**: Jahan efficiency aur optimal space complexity (O(N) vs O(N^2)) par bohot zyada focus hota hai.
- **Uber & Goldman Sachs**: Pair-finding aur distance-based optimization check karne ke liye ye standard problem hai.

---

### Why This Question?

Iska main maqsad aapki **Hashing for Index Tracking** ki samajh check karna hai:

1.  **Last Seen Index**: Kya aap ek `HashMap` maintain kar sakte hain jo har card ka **pichla position** store kare taaki aap turant gap calculate kar sakein?
2.  **Minimum Distance Logic**: Pura array traverse karte waqt sabse chota distance (`min_dist = min(min_dist, current_index - last_index + 1)`) dhoondhna.
3.  **One-Pass Solution**: Interviewer ye dekhta hai ki kya aap ek hi loop (O(N)) mein answer nikaal rahe hain ya nested loops use kar rahe hain.
...
> **Note:** Interviewer ye follow-up questions daag sakta hai:
>
> - "Agar humein shortest nahi, balki **Longest** distance nikaalna ho do same cards ke beech, toh logic mein kya change hoga?"
> - "Agar memory constraint ho aur hum `Map` use na kar sakein, toh kya sorting se kaam chalega? (Complexity par kya asar padega?)"
> - "Kya aap isse `Sliding Window` (two pointers) approach se solve kar sakte hain?"
